 ...
 * -------------------------------------------------------------
 * Phase 1 (now):   Render the form from static.json + booking.json.
 * Phase 2 (later): Replace booking.json with a live Bitrix24 fetch,
 *                  and compute installment amounts instead of reading them.
 */
...
/** Load fresh JSON on every request so edits show up without a restart. */
function loadJson(file) {
  const raw = fs.readFileSync(path.join(__dirname, "data", file), "utf-8");
  return JSON.parse(raw);
}

/** Format a number the way AED amounts appear: 1,234,567.89 (no symbol). */
function money(n) {
...
/**
 * PHASE-1 plan calc: only the summary figures (discount + net payable).
 * The full per-installment computation comes in phase 2 — for now the
 * row amounts are read straight from booking.json.
 */
